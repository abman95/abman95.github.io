...
export function dialogUpdater() {
    ...
    state.oneBeforePreviousIndexValue =
        state.previousIndices[state.oneBeforePreviousIndex];

    const newIndexContainer = document.querySelector(
        `div[data-index="${state.currentIndex}"]`,
    );
    ...
    const previousContainer = document.querySelector(
        `div[data-index="${state.oneBeforePreviousIndexValue}"]`,
    );
...
export function handlePreviousSongClick() {
    if (state.currentIndex === state.oneBeforePreviousIndexValue) return;
    const oneBeforePreviousContainer = document.querySelector(
        `div[data-index="${state.indexBeforePreviousButtonArrayPop}"]`,
    );
...
export function updatePlayPauseButton() {
    const iconSrc = state.audio.paused
        ? "./assets/images/playButton2.svg"
...
export function updateAutoPlayButton() {
    if (state.isAutoPlayEnabled) {
        ...
        elements.autoPlayIcon.setAttribute(
            "src",
            "./assets/images/autoplayButtonOn.svg",
        );
    } else {
        ...
        elements.autoPlayIcon.setAttribute(
            "src",
            "./assets/images/autoplayButtonOff.svg",
        );
    }
}
...
export function updateRandomPlayButton() {
    if (state.isRandomEnabled) {
        ...
        elements.randomSongIcon.setAttribute(
            "src",
            "./assets/images/shuffleButton.svg",
        );
    } else {
        ...
        elements.randomSongIcon.setAttribute(
            "src",
            "./assets/images/unshuffleButton2.svg",
        );
    }
}
